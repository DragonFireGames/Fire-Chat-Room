    ...
    for (var i in p.friends) (function(i){
      loadProfile(i,function(p2){
        ...
        if (p2.friends[user.name]) {
          ...
          p2.onclick = function(){
            var gid = randomId(8);
            notifyUser(p2.name,user.data.displayname+" challenged you to a match of Chess","",{game:gid});
            joinMatch(gid);
          };
        }
        return p2;
...
var thumbnail = loadImage("https://www.chess.com/bundles/web/images/offline-play/standardboard.1d6f9426.png");
pageMap.home.before = function() {
  beveledImage(thumbnail,5,30,170,170,theme.bevel);
  format(theme.announcement,false,0,14,LEFT,TOP);
  text("Playing: Chess\n - Elo: "+(user.data.chessElo||1200)+"\n - Wins: "+(user.data.chessWins||0)+"\n - Losses: "+(user.data.chessLosses||0)+"\n - Draws: "+(user.data.chessDraws||0),5,205);
};
...
function showChessProfile(color) {
  ...
  text((prof.displayname||prof.name)+" ("+(prof.chessElo||1200)+")",40,5);
  push();
...
function updateElo(winner) {
  ...
  var n = Elo.getNewRating(profileCache[data.white.name].chessElo||1200,profileCache[data.black.name].chessElo||1200,outcome);
  if (data.white.name == user.name) user.update({chessElo:n});
  if (winner == "draw") {
    user.update({chessDraws:(user.data.chessDraws||0)+1});
  } else if (data[winner].name == user.name) {
    user.update({chessWins:(user.data.chessWins||0)+1});
  } else {
    user.update({chessLosses:(user.data.chessLosses||0)+1});
  }
}
